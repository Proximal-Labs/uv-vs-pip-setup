...
# Sweep pip tasks across a set of OpenRouter models.
TASKS_DIR="tasks_pip"
...
docker_cleanup() {
  ...
  if [[ "$mode" == "scoped" ]]; then
    ...
    # Remove obvious OpenHands/pip-eval networks & volumes (best effort)
    docker network ls --format '{{.ID}} {{.Name}}' \
      | awk '/openhands|all-hands/ {print $1}' \
      | xargs -r docker network rm || true
    docker volume ls --format '{{.Name}}' \
      | awk '/openhands|pip-eval/ {print $1}' \
...
for m in "${MODELS[@]}"; do
  ...
  slug="$(slugify "$m")-pip-eval"
  echo "==> Running pip sweep for: $m"
...
echo "All pip runs queued. Check *_pip-eval.log files for details."
